...
class Map:
    ...
    def __init__(self, filename):
        ...
        if len(data[0]) != 1:
            print
            "Error! Map file line 1 must have exactly 1 integer"
            exit()
        ...
        if len(data) != self.numTowns:
            print
            "Error!: Map file must have the same number of input lines as the number of towns"
            exit()
        ...
        for i in xrange(1, self.numTowns):
            if len(data[i]) != i:
                print
                "Error! Map file line " + str(i + 1) + " must have exactly " + str(i) + " integer" + (
                    "s" if i != 1 else "")
                error = True
            for j in xrange(i):
                if data[i][j] < 1:
                    print
                    "Error! Map file line " + str(i + 1) + " integer " + str(
...
if len(argv) != 2:
    ...
    print
    "Usage: python checkTSP.py MAP_FILE_NAME"
    exit()
